...
function SignedXml(idMode, options) {
  ...
  (this.keyInfoProvider = true),
    (this.pixKeyInfo = null),
...
SignedXml.prototype.calculateSignatureValue = function (doc, callback) {
  var signedInfoCanon = this.getCanonSignedInfoXml(doc);
  var signer = this.findSignatureAlgorithm(this.signatureAlgorithm);
  this.signatureValue = signer.getSignature(
    signedInfoCanon,
    this.signingKey,
    callback
  );
...
SignedXml.prototype.validateReferences = function (doc) {
  for (var r in this.references) {
    ...
    let uri;
    if (ref.uri !== null) {
      uri = ref.uri[0] == "#" ? ref.uri.substring(1) : ref.uri;
    } else {
      uri = null;
    }
    var elem = [];
    ...
    if (uri === "") {
      let ehSpi = false;
      for (const spiRef in this.references) {
        if (this.references[spiRef].uri === null) {
          ehSpi = true;
        }
      }
      if (ehSpi) {
        elem = xpath.select('//*[local-name(.)="AppHdr"]', doc);
      } else {
        elem = xpath.select("//*", doc);
      }
    } else if (uri === null) {
      elem = xpath.select('//*[local-name(.)="Document"]', doc);
...
SignedXml.prototype.loadReference = function (ref) {
    ...
    if (inclusiveNamespaces.length > 0) {
      ...
      for (var i = 0; i < inclusiveNamespaces.length; i++) {
        if (inclusiveNamespacesPrefixList) {
          ...
          inclusiveNamespacesPrefixList = inclusiveNamespaces[i].getAttribute(
            "PrefixList"
          );
        }
      }
  ...
  let refuri;
  if (utils.findAttr(ref, "URI") === null) {
    refuri = null;
  } else {
    refuri = utils.findAttr(ref, "URI").value;
  }
...
SignedXml.prototype.addPixKeyInfo = function (
  transforms,
  digestAlgorithm,
  uri,
  IssuerName,
...
SignedXml.prototype.getKeyInfo = function (prefix) {
  ...
  if (this.keyInfoProvider) {
    res += `<ds:KeyInfo id="${this.pixKeyInfo.uri}">`;
    res += getKeyInfoData(this.signingKey, prefix);
    res += "</ds:KeyInfo>";
  }
  return res;
};

getKeyInfoData = function () {
  return (
    "<ds:X509Data><ds:X509IssuerSerial><ds:X509IssuerName>" +
    this.pixKeyInfo.IssuerName +
    "</ds:X509IssuerName><ds:X509SerialNumber>" +
    this.pixKeyInfo.SerialNumber +
...
SignedXml.prototype.createReferences = function (doc, prefix) {
  ...
  for (var n in this.references) {
    ...
    for (var h in nodes) {
      ...
      } else if (!ref.isEmptyUri && ref.hasUri) {
  ...
  if (this.pixKeyInfo) {
    pixKeyInfo = this.pixKeyInfo;
    res += "<" + prefix + 'Reference URI="#' + pixKeyInfo.uri + '">';

    res += "<" + prefix + "Transforms>";
    for (var t in pixKeyInfo.transforms) {
      if (!pixKeyInfo.transforms.hasOwnProperty(t)) continue;
      var trans = pixKeyInfo.transforms[t];
      var transform = this.findCanonicalizationAlgorithm(trans);
      res +=
        "<" +
        prefix +
    ...
    const nodes = xpath.select(
      '//*[local-name(.)="KeyInfo"]',
      new Dom().parseFromString(this.getKeyInfo(prefix))
    );

    const trransform = this.findCanonicalizationAlgorithm(
      pixKeyInfo.transforms[0]
    );
    const canonXml = trransform.process(nodes[0].cloneNode(true), {});

    const digestAlgorithm = this.findHashAlgorithm(pixKeyInfo.digestAlgorithm);
