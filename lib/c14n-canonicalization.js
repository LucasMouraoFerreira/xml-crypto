...
C14nCanonicalization.prototype.renderAttrs = function (node, defaultNS) {
  ...
  for (a in attrListToRender) {
    ...
    res.push(
      ...
      attr.name,
      '="',
      utils.encodeSpecialCharactersInAttribute(attr.value),
      '"'
    );
  }
    ...
    for (var p1 in ancestorNamespaces) {
      ...
      for (var p2 in nsListToRender) {
        if (
          ...
          nsListToRender[p2].namespaceURI ===
            ancestorNamespaces[p1].namespaceURI
        ) {
          alreadyListed = true;
        }
      }
...
C14nCanonicalizationWithComments.prototype = Object.create(
  C14nCanonicalization.prototype
);
